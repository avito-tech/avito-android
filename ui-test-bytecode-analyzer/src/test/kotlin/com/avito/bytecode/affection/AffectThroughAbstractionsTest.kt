...
class AffectThroughAbstractionsTest {
    ...
    fun `target class affected through abstraction - when this abstraction called in test`() {
        ...
        assertThat(invocations[PAGE_1] as Set<String>)
            .contains("$ABSTRACTION_AFFECT_TEST_CLASS.should_use_page1_and_page2_because_abstraction_is_used_that_has_implementations_that_affects_this_pages")
    ...
    fun `target class affected through nested abstraction - when this abstraction called in test`() {
        ...
        assertThat(invocations).containsKey(PAGE_1)
        assertThat(invocations[PAGE_1] as Set<String>)
            .contains("$ABSTRACTION_AFFECT_TEST_CLASS.should_use_page1_and_page2_because_nested_abstraction_is_used_that_has_implementations_that_affects_this_pages")
    ...
    fun `implementations of base interfaces are not affected - when method called on second interface level method`() {
        ...
        assertThat(invocations).containsKey(PAGE_1)
        assertThat(invocations[PAGE_1] as Set<String>)
            .contains("$ABSTRACTION_AFFECT_TEST_CLASS.should_use_page1_because_nested_implementation_has_just_page1_invocation")
    }
}
