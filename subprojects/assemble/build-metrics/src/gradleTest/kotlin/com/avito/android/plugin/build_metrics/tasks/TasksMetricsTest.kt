...
internal class TasksMetricsTest : BaseTasksMetricsTest(
    ...
    @Test
    fun `build - send tasks metrics - cc disabled`() {
        val result = build(":lib:compileKotlin", configurationCache = false)
        ...
        // available only without configuration cache
    ...
    @Test
    fun `build - send tasks metrics - cc enabled`() {
        checkWithCC()
    }

    @Test
    fun `repeatedly build - send tasks metrics - cc enabled`() {
        checkWithCC()
        checkWithCC()
    }

    private fun checkWithCC() {
        val result = build(
            ":lib:compileKotlin",
        )
