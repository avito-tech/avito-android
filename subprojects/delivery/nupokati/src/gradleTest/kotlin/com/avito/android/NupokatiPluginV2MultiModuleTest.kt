...
internal class NupokatiPluginV2MultiModuleTest {
    ...
    init {
        mockDispatcher.registerMock(
            Mock(
                requestMatcher = {
                    method == "PUT" && path.contains("release.aab")
                },
        ...
        mockDispatcher.registerMock(
            Mock(
                requestMatcher = {
                    method == "POST" && path == "/qapps/api/os/android/upload"
                },
                response = MockResponse().setResponseCode(HttpCodes.OK)
            )
        )
    ...
    @Test
    fun `uploadCdBuildResult - collects artifacts from dependent project`(@TempDir projectDir: File) {
        val cdConfig = """
            |{
            |  "schema_version": 2,
            |  "project": "avito",
        ...
        |    {
        |      "type": "bundle",
        |      "name": "app2-release.aab",
        |      "uri": "${mockWebServerUrl}artifactory/mobile-releases/avito_android/118.0_2/app2-release.aab",
        |      "build_variant": "release"
        |    }
        |  ]
        |}
            ...
            modules = listOf(
                AndroidAppModule(
                    ...
                    buildGradleExtra = """
                            ...
                            |
                            |dependencies {
                            |    "releaseArtifacts"(project(":app2", "releaseArtifactsElements"))
                            |}
                            |""".trimMargin()
                ),
                AndroidAppModule(
